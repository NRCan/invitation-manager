	...
	function mainPart1 () {
		...
		// Case when the user has already seen an invitation
			...
			if (isStorageExpired(lastDateIMShown)) {
				consoleLog("lastDateIMShown - Expired. Clearing.");
		...
		for (var i = 0; i < list.length; i++) {
			...
			if (l !== undefined && l.length !== undefined) {
				if (l.toLowerCase() == h.toLowerCase()) {
					consoleLog("On blacklist. h:" + h + " l:" + l);
					return false;
				}
			}
		}
		...
		for (var i = 0; i < surveyDB.surveys.length; i++) {
			...
			switch(survey_i.type) {
				...
				case "Page":
					...
					console.log('wLocation', wLocation);

					for (var c = 0; c < sublist.length; c++) {
						if  (c==0) {
							console.log("test", wLocation, sublist[c], sublist[c].url.toLowerCase());
						}
						if (wLocation.indexOf(sublist[c].url.toLowerCase()) !== -1 && i>=0) {
							console.log('here', c, sublist[c].url.toLowerCase());
